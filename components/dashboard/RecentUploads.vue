...
<template>
  <Card class="h-full">
    ...
    <div v-if="sermons.length" class="space-y-3">
      ...
      >
        <div class="flex-shrink-0 bg-pink-500 text-white text-xs font-bold rounded-lg px-2 py-1.5 text-center min-w-[64px]">
          <p class="text-sm leading-tight">{{ sermon.videoAttendees ?? 0 }}</p>
          <p class="text-[10px] font-medium">attendees</p>
        </div>
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-gray-800 leading-tight line-clamp-1">{{ sermon.topic }}</p>
    ...
    <NuxtLink to="/admin/teachings" class="block text-center text-xs text-blue-600 hover:underline mt-4">
      View all →
    </NuxtLink>
  </Card>
</template>
