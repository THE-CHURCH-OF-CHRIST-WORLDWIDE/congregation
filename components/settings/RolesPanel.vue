...
function openCustom(assignmentId: string) {
  ...
  customPerms.value = JSON.parse(JSON.stringify({
    ...basePerms,
  ...
  <div class="flex flex-col gap-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">Define what each role can access and assign roles to members.</p>
      </div>
    ...
    <div>
      <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Role Definitions</p>
      <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
        ...
        >
          <div class="flex items-start justify-between mb-3">
            ...
            >
              <Icon icon="mdi:shield-account-outline" class="text-lg" :style="{ color: role.color }" />
            </div>
          ...
          <p class="text-xs text-blue-500 mt-3 font-medium group-hover:underline">Edit permissions →</p>
        </button>
      </div>
    </div>
    ...
    <div>
      <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Member Assignments</p>
      <Card padding="none">
          ...
          <table class="w-full text-sm" role="table">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-4 py-3">
                  <Badge v-if="a.customPermissions && Object.keys(a.customPermissions).length" variant="warning" size="sm">
  ...
  >
    <div v-if="selectedRole" class="flex flex-col gap-4">
          ...
          <tbody>
            ...
            >
              ...
              >
                ...
                >
                  <Icon v-if="editPerms[page]?.[action]" icon="mdi:check" class="text-white text-xs" />
                </button>
              </td>
      ...
      <p class="text-xs text-gray-400">
        Click column headers to toggle all pages for that action. Click "All/None" on a row to toggle all actions for that page.
        Enabling any action auto-enables View. Disabling View clears all.
      </p>
    </div>
  ...
  <Modal v-model="showAssign" title="Assign Role to Member" size="md">
    <div class="flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-1">
        <label class="text-sm font-medium text-gray-700">Member<span class="text-red-500 ml-0.5">*</span></label>
        <div class="relative">
        ...
        <div v-if="memberSearch || filteredMembers.length" class="mt-1 border border-gray-200 rounded-lg overflow-hidden max-h-40 overflow-y-auto">
          <button
            v-for="m in filteredMembers"
            :key="m.id"
            :class="[
              'w-full text-left flex items-center gap-2.5 px-3 py-2 text-sm hover:bg-blue-50 transition-colors',
              assignForm.memberId === m.id ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700',
            ]"
            @click="assignForm.memberId = m.id; memberSearch = m.name"
          >
            ...
            <Icon v-if="assignForm.memberId === m.id" icon="mdi:check-circle" class="ml-auto text-blue-600" />
          </button>
          <p v-if="!filteredMembers.length" class="text-center py-4 text-xs text-gray-400">No members found</p>
        </div>
      ...
      <!-- Role picker -->
      <div class="flex flex-col gap-1">
        <label class="text-sm font-medium text-gray-700">Role<span class="text-red-500 ml-0.5">*</span></label>
        <div class="grid grid-cols-2 gap-2">
          <button
            ...
            :style="assignForm.roleId === role.id ? { borderColor: role.color, backgroundColor: role.color + '10' } : {}"
          ...
          >
            ...
            >
              <Icon icon="mdi:shield-account-outline" class="text-sm" :style="{ color: role.color }" />
            </div>
      ...
      <div
        ...
        :style="{ backgroundColor: (rolesStore.roleById(assignForm.roleId)?.color ?? '#6366f1') + '12' }"
      >
        <p class="font-semibold mb-0.5" :style="{ color: rolesStore.roleById(assignForm.roleId)?.color }">
          {{ rolesStore.roleById(assignForm.roleId)?.name }}
        </p>
        <p>{{ rolesStore.roleById(assignForm.roleId)?.description }}</p>
        <p class="mt-1 text-gray-400">
          {{ permCount(rolesStore.roleById(assignForm.roleId)?.permissions ?? {}) }} permissions across
          ...
          <tbody>
            ...
            >
              ...
              >
                ...
                >
                  <Icon v-if="customPerms[page]?.[action]" icon="mdi:check" class="text-white text-xs" />
                </button>
              </td>
            </tr>
          </tbody>
