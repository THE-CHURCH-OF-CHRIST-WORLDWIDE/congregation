...
const draft = ref({ ...store.settings, activities: [...store.settings.activities], leaders: store.settings.leaders.map(l => ({ ...l })), galleryPhotos: store.settings.galleryPhotos.map(p => ({ ...p })), congregationPhotos: [...store.settings.congregationPhotos] })
watch(() => store.settings, (s) => {
  draft.value = { ...s, activities: [...s.activities], leaders: s.leaders.map(l => ({ ...l })), galleryPhotos: s.galleryPhotos.map(p => ({ ...p })), congregationPhotos: [...s.congregationPhotos] }
}, { deep: true })
...
<template>
  <div class="flex flex-col gap-5">
    ...
    <Transition name="fade">
      <div v-if="saved" class="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">
        <Icon icon="mdi:check-circle-outline" />
        Settings saved successfully!
      </div>
    </Transition>
    <Transition name="fade">
      <div v-if="saveError" class="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
    ...
    <Transition name="fade" mode="out-in">
      ...
      <div v-if="activeTab === 'general'" key="general" class="flex max-w-2xl flex-col gap-5">
        <Card>
          ...
          <div class="flex flex-col gap-4">
            ...
            <Input v-model="draft.address" label="Address" placeholder="7B Esa Atan Ext. Ikot Ekpene" />
            <Input v-model="draft.phone" label="Phone Number" placeholder="(+234) 900 000 0000" />
            <Input v-model="draft.email" label="Email Address" placeholder="info@churchofchrist.org" />
          </div>
        </Card>
      ...
      <div v-else-if="activeTab === 'homepage'" key="homepage" class="flex max-w-2xl flex-col gap-5">
        <Card>
          <h3 class="mb-4 text-sm font-semibold text-gray-900">Hero Section</h3>
          <div class="flex flex-col gap-4">
            <Input v-model="draft.heroImageUrl" label="Building Photo URL" placeholder="https://…/church-building.jpg" helper="Shown as the full-width background image in the hero card." />
        ...
        <Card>
          ...
          <div class="flex flex-col gap-4">
            ...
            >
              <Input v-model="act.name" label="Activity Name" placeholder="Sunday Worship Service" />
              <Input v-model="act.timeRange" label="Time Range" placeholder="9:00 am - 12:00 noon" />
            ...
            <p v-if="!draft.activities.length" class="text-center text-sm text-gray-400">No activities added yet.</p>
          </div>
        </Card>
      ...
      <!-- ── Minister Welcome ─────────────────────────────────────────── -->
      <div v-else-if="activeTab === 'minister'" key="minister" class="flex max-w-2xl flex-col gap-5">
        <Card>
          <h3 class="mb-4 text-sm font-semibold text-gray-900">Minister Details</h3>
          <div class="flex flex-col gap-4">
            <Input v-model="draft.ministerName" label="Minister Name" placeholder="Min. Friday Asuquo" />
            <Input v-model="draft.ministerTitle" label="Title / Role" placeholder="Resident Minister" />
            <Input v-model="draft.ministerPhoto" label="Minister Photo URL" placeholder="https://…/minister.jpg" />
            <div v-if="draft.ministerPhoto" class="flex items-center gap-3">
              <img :src="draft.ministerPhoto" alt="Minister preview" class="h-20 w-20 rounded-full object-cover border border-gray-200" />
            </div>
          </div>
        </Card>
        ...
        <Card>
          ...
          <p class="mb-3 text-xs text-gray-400">These appear as the stacked "fan" cards behind the minister portrait.</p>
          <div class="flex flex-col gap-3">
            <Input v-model="draft.congregationPhotos[0]" label="Back Card Photo URL" placeholder="https://…/congregation1.jpg" />
            <Input v-model="draft.congregationPhotos[1]" label="Mid Card Photo URL" placeholder="https://…/congregation2.jpg" />
          </div>
        </Card>
        ...
        <Card>
          ...
          <div class="flex flex-col gap-4">
            <div>
              <label for="letter-p1" class="mb-1.5 block text-sm font-medium text-gray-700">Paragraph 1 (opening, with drop-cap "D")</label>
              <textarea id="letter-p1" v-model="draft.ministerLetterP1" rows="3" class="w-full resize-none rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"></textarea>
            </div>
            <div>
              <label for="letter-p2" class="mb-1.5 block text-sm font-medium text-gray-700">Paragraph 2</label>
              <textarea id="letter-p2" v-model="draft.ministerLetterP2" rows="4" class="w-full resize-none rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"></textarea>
            </div>
            <div>
              <label for="letter-p3" class="mb-1.5 block text-sm font-medium text-gray-700">Paragraph 3</label>
              <textarea id="letter-p3" v-model="draft.ministerLetterP3" rows="3" class="w-full resize-none rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"></textarea>
            </div>
            <div>
              <label for="letter-p4" class="mb-1.5 block text-sm font-medium text-gray-700">Paragraph 4 (closing)</label>
              <textarea id="letter-p4" v-model="draft.ministerLetterP4" rows="2" class="w-full resize-none rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"></textarea>
            </div>
          </div>
        </Card>
          ...
          <div class="flex flex-col gap-3">
            ...
            >
              ...
              <div class="h-10 w-10 shrink-0 overflow-hidden rounded-full bg-gray-100">
                ...
                <div v-else class="flex h-full w-full items-center justify-center text-xs font-bold text-gray-400">?</div>
              </div>
              <Input v-model="leader.name" label="Name" placeholder="Akpan Lincoln" />
              <div>
                <label :for="`role-${leader.id}`" class="mb-1.5 block text-sm font-medium text-gray-700">Role</label>
                <select
                  :id="`role-${leader.id}`"
                  v-model="leader.role"
            ...
            <p v-if="!draft.leaders.length" class="py-4 text-center text-sm text-gray-400">No leaders added yet.</p>
          </div>
        </Card>
      ...
      <div v-else-if="activeTab === 'gallery'" key="gallery" class="flex max-w-3xl flex-col gap-5">
        <Card>
          ...
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            >
              ...
              <div class="flex flex-col gap-2">
                ...
                <Input v-model="img.alt" label="Caption / Alt text" placeholder="Sunday worship service" />
              </div>
              <button
                class="absolute right-2 top-2 rounded p-0.5 text-gray-400 hover:bg-red-50 hover:text-red-500"
          ...
          <p v-if="!draft.galleryPhotos.length" class="py-4 text-center text-sm text-gray-400">No photos added yet.</p>
        </Card>
